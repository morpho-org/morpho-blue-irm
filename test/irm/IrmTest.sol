...
contract IrmTest is Test {
    ...
    constructor() {
        irm = new Irm(address(this), ln2, WAD, 0.8 ether);
    }
    ...
    function testWPow(int256 x) public {
        assertApproxEqRel(wPow(ln2, -1 ether), 0.5 ether, 0.02 ether);
        assertApproxEqRel(wPow(ln2, -0.5 ether), 0.70710678118 ether, 0.01 ether);
        assertEq(wPow(ln2, 0), 1 ether);
        assertApproxEqRel(wPow(ln2, 0.5 ether), 1.41421356237 ether, 0.01 ether);
        assertApproxEqRel(wPow(ln2, 1 ether), 2 ether, 0.02 ether);
